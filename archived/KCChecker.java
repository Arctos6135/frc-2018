...
public class OI {
	...
	static class FixedSizeQueue<T> extends ArrayDeque<T> {
		int len;
		public FixedSizeQueue(int size, Class<T> type) {
			super(size);
			len = size;
			while(this.size() < len)
				try {
					super.addLast(type.newInstance());
	...
	static FixedSizeQueue<String> lastInputs = new FixedSizeQueue<String>(10, String.class);
	static String[] code = new String[] {
			"0", "0", "180", "180", "270", "90", "270", "90", "b", "a"
	};
	static void checkCode() {
		int index = 0;
		for(String s : lastInputs) {
			if(!s.equalsIgnoreCase(code[index++]))
				return;
		}
		
		//Do some stuff
		Robot.printInfo = !Robot.printInfo;
	}
	static void initCodeDetector() {
		Trigger povDetector = new Trigger() {
			int lastDirection = -1;
			@Override
			public boolean get() {
				int pov = driveController.getPOV();
				if(pov != lastDirection && pov != -1) {
					lastInputs.addLast(String.valueOf(pov));
				}
				lastDirection = pov;
				checkCode();
				return false;
		...
		povDetector.whenActive(new InstantCommand());
		JoystickButton bButton = new JoystickButton(driveController, RobotMap.ControllerMap.BUTTON_B);
		JoystickButton aButton = new JoystickButton(driveController, RobotMap.ControllerMap.BUTTON_A);
		JoystickButton xButton = new JoystickButton(driveController, RobotMap.ControllerMap.BUTTON_X);
		JoystickButton yButton = new JoystickButton(driveController, RobotMap.ControllerMap.BUTTON_Y);
		
		bButton.whenPressed(new InstantCommand() {
			@Override
			protected void initialize() {
				lastInputs.addLast("b");
				checkCode();
			}
		});
